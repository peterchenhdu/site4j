<#-- 公共顶部 -->
<#macro header title="SITE4J" keywords="" description="" canonical="">
<!DOCTYPE HTML>
<html lang="zh-CN">
<head>
    <meta charset="utf-8">
    ...
    <meta name="sogou_site_verification" content="sYJUpM6qYM"/>
    <title>${title}</title>
...
<#macro pageBar>
    ...
    <nav>
        <ul class="pager page-btn" data-url="${config.siteUrl}/${url?if_exists}"
            ...
            <#list 1..page.pages as item>
                <li>
                    <a ${(page.pageNum == item)?string('class="pointer active"','class="pointer" data-page="${item}"')}>${item}</a>
                </li>
            </#list>
            <#if page.hasNextPage>
